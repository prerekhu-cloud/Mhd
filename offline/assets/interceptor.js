...
// 3D asset redirects per city+asset → GitHub URL (null = pass through to real server)
var GH_3D_ASSETS = {
  opava: {
...
// Returns stripped inline era (no paths) — used for GTFS simulation
...
// Fetches full era (with route paths) from GitHub — used for map rendering
...
window.fetch = async function(input, init){
  ...
  // 3D static assets (/cities/{slug}/{asset}.json) → GitHub
  ...
  if(m3d){
    ...
    if(cityMap && asset in cityMap){
      ...
      if(ghUrl) return _origFetch(ghUrl, init);
      return _origFetch(input, init); // null = pass through
    }
...
console.info('[MHD] Offline mode — API + 3D intercepted, era data from GitHub');
})();
